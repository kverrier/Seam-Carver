 ...
 * algorithm picks a continuous seam of uninteresting pixels and removes them in
 * order to resize images.
 * 
...
public class SeamCarvingFilter {
	 ...
	 * 
	 * @param original
	 *            Image to remove column from
	 * @return A new image that is one column smaller than the original
	 * @throws IllegalArgumentException
	 *             if the original image is invalid or too small
	 * @throws NullPointerException
	 *             if the original image pointer is null
	 */
	public BufferedImage removeColumn(final BufferedImage original)
		...
		float[] garbArray = new float[original.getWidth() * 3]; // Garbage array
																// that's
		// needed by the
		// getPixels function
		for (int row = 0; row < seam.size(); ++row) {
			...
			if (curCol > 0) {
				...
				raster.setPixels(0, row, curCol, 1,
						origData.getPixels(0, row, curCol, 1, garbArray));
			}
			...
			if (seam.get(row) < original.getWidth() - 1) {
				...
				raster.setPixels(curCol, row, widthAfter, 1, origData
						.getPixels(curCol + 1, row, widthAfter, 1, garbArray));
			}
		}
	 ...
	 * 
	 * @param original
	 *            Image to calculate the seam for
